...
export class ObstacleManager {
  ...
  makeObstacle(){
    ...
    if(type === 'box'){
      mesh = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({color:0xff3b3b, metalness:0.1, roughness:0.8, emissive:0x220000, emissiveIntensity:0.02})
      );
    } else if(type === 'gate'){
  ...
  spawn(){
    ...
    this.spawnAt(lane, this.spawnZ);
  }

  spawnAt(lane, z){
    const o = this.pool.pop() || this.makeObstacle();
    o.position.set(lane, 0.6, z);
    o.visible = true;
    this.active.push(o);
  }
